...
func (ctrl *ApiTagController) Get() {
	...
	out, len, err := ctrl.BaseController.TagGet(limit, site_id, site_flag, channel_id)
	if err != nil {
		logs.Error("Get::", err)
...
func (ctrl *ApiTagController) GetNew() {
	site_flag := ctrl.GetSafeString("site_flag")
	site_id, _ := ctrl.GetInt64("site_id")
	channel_id, _ := ctrl.GetInt64("channel_id")
	limit, _ := ctrl.GetInt("limit", 6)
	out, len, err := ctrl.BaseController.TagGetNew(limit, site_id, site_flag, channel_id)
	if err != nil {
		logs.Error("GetNew::", err)
		ctrl.JSONErrorOfData(err.Error(), out)
	}
	ctrl.JSONSuccess(strconv.FormatInt(len, 10), out)
}
...
func (ctrl *ApiTagController) Click() {
	tag_id, _ := ctrl.GetInt64("tag_id", 0)
	err := ctrl.BaseController.TagClick(tag_id)
	if err != nil {
...
func (ctrl *ApiTagController) ArticlePaginate() {
	limit, _ := ctrl.GetInt("limit", 6)
	...
	tag_name := ctrl.GetSafeString("tag_name")
	...
	outArticle, count, err := ctrl.BaseController.TagArticlePaginate(page, limit, tag_name, channel_id, channel_name, category_id, call_index, keyword, is_top, is_red, is_hot, is_slide, is_search, order_by)
	if err != nil {
