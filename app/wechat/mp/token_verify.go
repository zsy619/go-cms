...
func (this *TokenVerify) Verify() (string, error) {
	...
	// if len(this.EchoStr) == 0 {
	// 	return "", errors.New("echostr为空")
	// }
	signatureGen := this.MakeSignature(this.Timestamp, this.Nonce)
	if signatureGen == this.Signature {
		return this.EchoStr, nil
	}
	return "", errors.New("signature验证错误")
}

func (this *TokenVerify) MakeSignature(timestamp, nonce string) string {
