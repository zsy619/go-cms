...
// TagPaginate 分页查询
func (svc *CmsTag) TagPaginate(page, limit int, channelId int64, name, title string, status int32, siteId ...int64) ([]*domain.CmsTag, int64, error) {
	mdl, do := mapper.CmsTagDo()
	if len(siteId) > 0 {
...
// TagFind 获取
func (svc *CmsTag) TagFind(tagId int64) (*domain.CmsTag, error) {
	mdl, do := mapper.CmsTagDo()
	return do.Where(mdl.TagID.Eq(tagId)).First()
}

// TagSave 保存或更新
func (svc *CmsTag) TagSave(input *domain.CmsTag) error {
	mdl, do := mapper.CmsTagDo()
	if input.Name != "" {
...
// SiteGet 获取站点
func (svc *CmsTag) SiteGet(roleId int64, roleType string) ([]*domain.CmsSite, error) {
