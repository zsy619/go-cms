...
func (svc *CmsAdminNotice) NoticePaginate(page, limit int, title string, status int32, adminId int64, roleType string) ([]*domain.CmsAdminNotice, int64, error) {
	mdl, do := mapper.CmsAdminNoticeDo()
	if title != "" {
...
func (svc *CmsAdminNotice) NoticeShow() ([]*domain.CmsAdminNotice, error) {
	mdl, do := mapper.CmsAdminNoticeDo()
	return do.Where(mdl.Status.Eq(2)).Order(mdl.IsTop.Desc(), mdl.CreateTime.Desc()).Limit(6).Find()
}

// NoticeFind 通过notice_id获取详情
func (svc *CmsAdminNotice) NoticeFind(noticeId int64) (*domain.CmsAdminNotice, error) {
	mdl, do := mapper.CmsAdminNoticeDo()
...
// NoticeSave 保存
func (svc *CmsAdminNotice) NoticeSave(input *domain.CmsAdminNotice) error {
	if input.Title == "" {
