...
// AlbumPaginate 获取
func (svc *CmsAlbum) AlbumPaginate(page, limit int, tableName string, recordId int64, typeId int32, adminId int64, roleType string) ([]*domain.CmsAlbum, int64, error) {
...
func (svc *CmsAlbum) AlbumSearch(page, limit int, tableName, title, ext string, adminId int64, roleType string) ([]*domain.CmsAlbum, int64, error) {
	mdl, do := mapper.CmsAlbumDo()
	if tableName != "" {
...
// AlbumSave 保存或更新
func (svc *CmsAlbum) AlbumSave(input *domain.CmsAlbum) error {
	mdl, do := mapper.CmsAlbumDo()
	var err error
