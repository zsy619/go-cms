...
func (svc *Menu) MenuList(adminId int64) *MenuOuter {
	...
	_, adminRoleDo := mapper.CmsAdminRoleDo()
	role := domain.CmsAdminRole{}
...
ORDER BY a.sort_id`, role.RoleID)
	...
	_, adminNavDo := mapper.CmsAdminNavDo()
	navs := []*domain.CmsAdminNav{}
	if err := adminNavDo.UnderlyingDB().Raw(sqlNav).Scan(&navs).Error; err != nil {
		return outerMenu
	}

	parentNavs := []*domain.CmsAdminNav{}
	linq.From(navs).WhereT(func(s *domain.CmsAdminNav) bool {
		return s.ParentID == 0 && s.IsHide == 0
	}).OrderByT(func(s *domain.CmsAdminNav) int32 {
		return s.SortID
	}).ToSlice(&parentNavs)
...
func (svc *Menu) ChildMenu(navs []*domain.CmsAdminNav, parentId int64) ([]*MenuInfo, error) {
	if len(navs) == 0 {
		return nil, nil
	}
	outMenu := []*MenuInfo{}

	childNavs := []*domain.CmsAdminNav{}
	linq.From(navs).WhereT(func(s *domain.CmsAdminNav) bool {
