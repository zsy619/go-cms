...
func (svc *ApiLink) get(cackeKeyPrefix string, limit int, site_id int64, site_flag string, category_id int64, call_index string) ([]*service_model.ApiLinkModel, int64, error) {
	...
	if found, item := lib.LinkCache.Get(cacheKey); found {
		list := item.([]*service_model.ApiLinkModel)
		logs.Debug("LinkList[Cache]::", "cacheKey", cacheKey, "LinkList", list)
		return list, int64(len(list)), nil
	}
	outLink := []*service_model.ApiLinkModel{}

	_, linkDo := mapper.CmsLinkDo()
...
* @return {*}
 */
func (svc *ApiLink) Get(limit int, site_id int64, site_flag string, category_id int64, call_index string) ([]*service_model.ApiLinkModel, int64, error) {
...
* @return {*}
 */
func (svc *ApiLink) GetNew(limit int, site_id int64, site_flag string, category_id int64, call_index string) ([]*service_model.ApiLinkModel, int64, error) {
 ...
 * @return {*}
 */
func (svc *ApiLink) Paginate(page, limit int, site_id int64, site_flag string, category_id int64, call_index string) ([]*service_model.ApiLinkModel, int64, error) {
	outLink := []*service_model.ApiLinkModel{}
	...
	sqlRow := "SELECT " + sqlSelectRow + " FROM cms_link a" +
		" LEFT JOIN cms_link_category b ON a.category_id = b.category_id" +
		" LEFT JOIN cms_site c ON a.site_id = c.site_id" +
		where +
	...
	sqlCount := "SELECT " + sqlSelectCount + " FROM cms_link a" +
		" LEFT JOIN cms_link_category b ON a.category_id = b.category_id" +
		" LEFT JOIN cms_site c ON a.site_id = c.site_id" +
		where
