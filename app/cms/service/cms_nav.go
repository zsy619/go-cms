...
func (svc *CmsAdminNav) NavFind(navId int64) (*domain.CmsAdminNav, error) {
	mdl, do := mapper.CmsAdminNavDo()
...
func (svc *CmsAdminNav) NavDestroy(navId int64) error {
	...
	// 判断是否关联站点
	sitMdl, siteDo := mapper.CmsSiteDo()
...
// NavTree 获取树形结构列表
func (svc *CmsAdminNav) NavTree(navId int64) ([]*service_model.TreeNode, error) {
...
func (svc *CmsAdminNav) NavTreeByParentId(parentId, navId int64) ([]*service_model.TreeNode, error) {
	out := make([]*service_model.TreeNode, 0)
	navMdl, navDo := mapper.CmsAdminNavDo()
...
// NavSave 保存导航详情
func (svc *CmsAdminNav) NavSave(input *domain.CmsAdminNav) error {
	if input.ParentID < 0 {
