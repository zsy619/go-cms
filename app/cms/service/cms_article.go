 ...
 * @return {*}
 */
func (svc *CmsArticle) ArticlePaginate(page, limit int, channelId, categoryId int64, title, callIndex string, status int32) ([]*domain.CmsArticle, int64, error) {
 ...
 * @return {*}
 */
func (svc *CmsArticle) ArticleFind(articleId int64) (*domain.CmsArticle, error) {
	mdl, do := mapper.CmsArticleDo()
 ...
 * @return {*}
 */
func (svc *CmsArticle) ArticleSave(input *domain.CmsArticle) error {
	mdl, do := mapper.CmsArticleDo()
	if input.CallIndex != "" {
	...
	{
		chnMdl, chnDo := mapper.CmsSiteChannelDo()
		var siteId int64
...
func (svc *CmsArticle) ArticleDestory(articleId int64) error {
	...
	proMdl, proDo := mapper.CmsArticlePropertyDo()
	if _, err := proDo.Where(proMdl.ArticleID.Eq(articleId)).Delete(); err != nil {
		return err
	}
	mdl, do := mapper.CmsArticleDo()
 ...
 * @return {*}
 */
func (svc *CmsArticle) CategoryPaginate(page, limit int, channelId int64, title, callIndex string) ([]*domain.CmsArticleCategory, int64, error) {
 ...
 * @return {*}
 */
func (svc *CmsArticle) CategoryFind(categoryId int64) (*domain.CmsArticleCategory, error) {
	mdl, do := mapper.CmsArticleCategoryDo()
 ...
 * @return {*}
 */
func (svc *CmsArticle) CategorySave(input *domain.CmsArticleCategory) error {
	mdl, do := mapper.CmsArticleCategoryDo()
	if input.CallIndex != "" {
...
func (svc *CmsArticle) CategoryAutoUrl(channelId int64) error {
	...
	channelMdl, channelDo := mapper.CmsSiteChannelDo()
...
func (svc *CmsArticle) CategoryDestory(categoryId int64) error {
	...
	artMdl, artDo := mapper.CmsArticleDo()
 ...
 * @return {*}
 */
func (svc *CmsArticle) CategoryTree(channelId, categoryId int64) ([]*service_model.TreeNode, error) {
	out := make([]*service_model.TreeNode, 0)
	mdl, do := mapper.CmsArticleCategoryDo()
 ...
 * @return {*}
 */
func (svc *CmsArticle) CategoryTreeByParentId(parentId, categoryId int64) ([]*service_model.TreeNode, error) {
	out := make([]*service_model.TreeNode, 0)
	mdl, do := mapper.CmsArticleCategoryDo()
