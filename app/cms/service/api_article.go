 ...
 * @return {*}
 */
func (svc *ApiArticle) CategoryNav(channel_name string, channel_id int64, call_index string, category_id int64, article_id int64) ([]*service_model.ApiCategoryNav, error) {
	...
	if found, item := lib.CategoryNavCache.Get(cacheKey); found {
		return item.([]*service_model.ApiCategoryNav), nil
	}
	outResult := []*service_model.ApiCategoryNav{}
	// 获取 频道信息
	{
		mdl, do := mapper.CmsSiteChannelDo()
		channel := &domain.CmsSiteChannel{}
		if channel_name != "" {
		...
		if channel != nil {
			channelItem := &service_model.ApiCategoryNav{
				Title:     channel.Title,
	...
	// 获取 栏目信息
	{
		...
		category := &domain.CmsArticleCategory{}
		if call_index != "" {
		...
		if category != nil {
			categoryItem := &service_model.ApiCategoryNav{
 ...
 * @return {*}
 */
func (svc *ApiArticle) CategoryGet(channel_name, call_index string) ([]*service_model.ApiCategoryGetModel, int64, error) {
	...
	if found, item := lib.CategoryGetCache.Get(cacheKey); found {
		list := item.([]*service_model.ApiCategoryGetModel)
 ...
 * @return {*}
 */
func (svc *ApiArticle) CategoryFind(category_id int64, call_index string) (*service_model.ApiCategoryFindModel, error) {
	...
	if found, item := lib.CategoryFindCache.Get(cacheKey); found {
		return item.(*service_model.ApiCategoryFindModel), nil
	}
 ...
 * @return {*}
 */
func (svc *ApiArticle) ArticleGet(limit int, channel_id int64, channel_name string, category_id int64, call_index string, is_top, is_red, is_hot, is_slide int, order_by string) ([]*service_model.ApiArticleListModel, int64, error) {
	...
	if found, item := lib.ArticleCache.Get(cacheKey); found {
		list := item.([]*service_model.ApiArticleListModel)
		logs.Debug("ArticleList[Cache]::", "cacheKey", cacheKey, "ArticleList", list)
		return list, int64(len(list)), nil
	}
	list := make([]*service_model.ApiArticleListModel, 0)
	_, do := mapper.CmsArticleDo()

	sql := service_model.ApiArticleListModel_Table +
 ...
 * @return {*}
 */
func (svc *ApiArticle) ArticleGetNew(limit int, channel_id int64, channel_name string, category_id int64, call_index string, is_top, is_red, is_hot, is_slide int, order_by string) ([]*service_model.ApiArticleListModel, int64, error) {
	order_by = xgeneric.IFF(order_by == "", "a.sort_id", order_by)
	if limit <= 0 {
		limit = 10
	}
	...
	if found, item := lib.ArticleGetNewCache.Get(cacheKey); found {
		find := item.([]*service_model.ApiArticleListModel)
		return find, int64(len(find)), nil
	}
	outArticle := make([]*service_model.ApiArticleListModel, 0)
	_, do := mapper.CmsArticleDo()
	sql := service_model.ApiArticleListModel_Table +
		xgeneric.IFF(channel_id <= 0, "", " And b.channel_id="+strconv.FormatInt(channel_id, 10)) +
		xgeneric.IFF(channel_name == "", "", " And c.name='"+channel_name+"'") +
 ...
 * @return {*}
 */
func (svc *ApiArticle) ArticlePaginate(page, limit int, channel_id int64, channel_name string, category_id int64, call_index string, keyword string, is_top, is_red, is_hot, is_slide, is_search int, order_by string) ([]*service_model.ApiArticleListModel, int64, error) {
	order_by = xgeneric.IFF(order_by == "", "a.sort_id", order_by)
 ...
 * @return {*}
 */
func (svc *ApiArticle) ArticleFind(call_index string, article_id int64) (*service_model.ApiArticleOneModel, []*service_model.ApiAlbumModel, []*service_model.ApiAttachModel, []*service_model.ApiPropertyModel, error) {
	mdl, do := mapper.CmsArticleDo()
	if call_index != "" {
		if article_id <= 0 {
	...
	if err := do.Debug().UnderlyingDB().Raw(sql, article_id).Scan(&list).Error; err != nil {
		logs.Error("Get", err.Error())
		return list, []*service_model.ApiAlbumModel{}, []*service_model.ApiAttachModel{}, []*service_model.ApiPropertyModel{}, err
	}

	albumMdl, alblumDo := mapper.CmsAlbumDo()
	albums := []*service_model.ApiAlbumModel{}
	...
	attachMdl, attachDo := mapper.CmsAttachDo()
	attachs := []*service_model.ApiAttachModel{}
 ...
 * @return {*}
 */
func (svc *ApiArticle) PrevNext(call_index string, category_id, article_id int64) (prev *service_model.ApiArticlePrevNextModel, next *service_model.ApiArticlePrevNextModel) {
	_, do := mapper.CmsArticleDo()
	field := `a.*` +
 ...
 * @return {*}
 */
func (svc *ApiArticle) Article(call_index string, article_id int64) (*service_model.ApiArticleOneModel, error) {
	_, do := mapper.CmsArticleDo()
	field := `a.*,b.call_index as category_call_index,b.title as category_title,b.link_url as category_link_url,c.name as channel_name,c.title as channel_title,case when b.tmpl_dtl='' then c.tmpl_dtl else b.tmpl_dtl end tmpl_dtl`
	sql := `SELECT ` + field + ` FROM cms_article a LEFT JOIN cms_article_category b ON a.category_id = b.category_id LEFT JOIN cms_site_channel c ON a.channel_id = c.channel_id`
	list := &service_model.ApiArticleOneModel{}
 ...
 * @return {*}
 */
func (svc *ApiArticle) Album(call_index string, article_id int64, type_id int32) ([]*service_model.ApiAlbumModel, error) {
	if call_index != "" {
		article, articleDo := mapper.CmsArticleDo()
		_ = articleDo.Where(article.CallIndex.Eq(call_index), article.Status.Eq(2)).Pluck(article.ArticleID, &article_id)
	}
	albumMdl, alblumDo := mapper.CmsAlbumDo()
	albums := make([]*service_model.ApiAlbumModel, 0)
 ...
 * @return {*}
 */
func (svc *ApiArticle) Attach(call_index string, article_id int64, type_id int32) ([]*service_model.ApiAttachModel, error) {
	if call_index != "" {
		article, articleDo := mapper.CmsArticleDo()
		_ = articleDo.Where(article.CallIndex.Eq(call_index), article.Status.Eq(2)).Pluck(article.ArticleID, &article_id)
	}
	attachMdl, attachDo := mapper.CmsAttachDo()
	attachs := make([]*service_model.ApiAttachModel, 0)
 ...
 * @return {*}
 */
func (svc *ApiArticle) Property(page, limit int, parentId, articleId int64, callIndex, title string) ([]*domain.CmsArticleProperty, int64, error) {
