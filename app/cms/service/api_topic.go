...
func (svc *ApiTopic) get(cackeKeyPrefix string, limit int, site_id int64, site_flag string, channel_id int64) ([]*service_model.ApiTopicModel, int64, error) {
	if limit <= 0 {
		limit = 10
	}
	...
	if found, item := lib.TopicCache.Get(cacheKey); found {
		list := item.([]*service_model.ApiTopicModel)
		logs.Debug("TopicList[Cache]::", "cacheKey", cacheKey, "TopicList", list)
		return list, int64(len(list)), nil
	}

	list := make([]*service_model.ApiTopicModel, 0)
	_, do := mapper.CmsTopicDo()
 ...
 * @return {*}
 */
func (svc *ApiTopic) Get(limit int, site_id int64, site_flag string, channel_id int64) ([]*service_model.ApiTopicModel, int64, error) {
 ...
 * @return {*}
 */
func (svc *ApiTopic) GetNew(limit int, site_id int64, site_flag string, channel_id int64) ([]*service_model.ApiTopicModel, int64, error) {
 ...
 * @return {*}
 */
func (svc *ApiTopic) Find(topic_id int64, name string) (*service_model.ApiTopicModel, error) {
	cacheKey := fmt.Sprintf("ApiTopic_Find_%d_%s", topic_id, name)
	if found, item := lib.TopicFindCache.Get(cacheKey); found {
		model := item.(*service_model.ApiTopicModel)
	...
	_, do := mapper.CmsTopicDo()
	model := &service_model.ApiTopicModel{}
 ...
 * @return {*}
 */
func (svc *ApiTopic) ArticlePaginate(page, limit int, topic_name string, site_id int64, site_flag string, channel_id int64, channel_name string, category_id int64, call_index string, keyword string, is_top, is_red, is_hot, is_slide, is_search int, order_by string) ([]*service_model.ApiArticleListModel, int64, error) {
...
func (svc *ApiTopic) ArtilceTop(limit int, topic_name string) ([]*service_model.ApiArticleListModel, int64, error) {
	if limit <= 0 {
		limit = 10
	}
	...
	if err != nil {
		return []*service_model.ApiArticleListModel{}, 0, err
	}
	if len(list) > 0 {
