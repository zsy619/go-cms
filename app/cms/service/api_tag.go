...
func (svc *ApiTag) get(cackeKeyPrefix string, limit int, site_id int64, site_flag string, channel_id int64) ([]*service_model.ApiTagModel, int64, error) {
	if limit <= 0 {
		limit = 10
	}
	...
	if found, item := lib.ArticleCache.Get(cacheKey); found {
		list := item.([]*service_model.ApiTagModel)
		logs.Debug("TagList[Cache]::", "cacheKey", cacheKey, "TagList", list)
		return list, int64(len(list)), nil
	}

	outTags := make([]*service_model.ApiTagModel, 0)
	_, do := mapper.CmsTagDo()
 ...
 * @return {*}
 */
func (svc *ApiTag) Get(limit int, site_id int64, site_flag string, channel_id int64) ([]*service_model.ApiTagModel, int64, error) {
 ...
 * @return {*}
 */
func (svc *ApiTag) GetNew(limit int, site_id int64, site_flag string, channel_id int64) ([]*service_model.ApiTagModel, int64, error) {
 ...
 * @return {*}
 */
func (svc *ApiTag) ArticlePaginate(page, limit int, tag_name string, channel_id int64, channel_name string, category_id int64, call_index string, keyword string, is_top, is_red, is_hot, is_slide, is_search int, order_by string) ([]*service_model.ApiArticleListModel, int64, error) {
...
func (svc *ApiTag) ArtilceTop(limit int, tag_name string) ([]*service_model.ApiArticleListModel, int64, error) {
	if limit <= 0 {
		limit = 10
	}
