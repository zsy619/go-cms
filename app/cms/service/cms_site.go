...
func (svc *CmsSite) SiteOne(id int64) (*domain.CmsSite, error) {
	site, siteDo := mapper.CmsSiteDo()
...
func (svc *CmsSite) SiteSave(mdl *domain.CmsSite, domains []string, remarks []string) error {
	...
	siteDomain, domainDo := mapper.CmsSiteDomainDo()
	_, _ = domainDo.Where(siteDomain.SiteID.Eq(mdl.SiteID)).Delete()
	if domains != nil {
 ...
 * @return {*}
 */
func (svc *CmsSite) ChannelPaginate(page, limit int, siteId int64, name, title string) ([]*domain.CmsSiteChannel, int64, error) {
...
// ChannelFind 获取
func (svc *CmsSite) ChannelFind(channelId int64) (*domain.CmsSiteChannel, error) {
	mdl, do := mapper.CmsSiteChannelDo()
	return do.Where(mdl.ChannelID.Eq(channelId)).First()
}

// ChannelSave 保存或更新
func (svc *CmsSite) ChannelSave(input *domain.CmsSiteChannel) error {
	mdl, do := mapper.CmsSiteChannelDo()
	if input.Name != "" {
 ...
 * @return {*}
 */
func (svc *CmsSite) ChannelNav(input *domain.CmsSiteChannel) error {
	dt, _ := time.Parse("2006-01-02 15:04:05", "2023-03-20 00:00:00")
	// cms_admin_nav
	var siteNavId int64
	navMdl, navDo := mapper.CmsAdminNavDo()
	...
	if siteNavId > 0 {
		...
		if channelNavId > 0 {
			if count, _ := navDo.Where(navMdl.ParentID.Eq(channelNavId)).Count(); count == 0 {
				...
				navCategory := &domain.CmsAdminNav{
					SiteID:     input.SiteID,
					ChannelID:  input.ChannelID,
					ParentID:   channelNavId,
					Type:       "Article",
 ...
 * @return {*}
 */
func (svc *CmsSite) ChannelTree(siteId, channelId int64) ([]*service_model.TreeNode, error) {
 ...
 * @return {*}
 */
func (svc *CmsSite) ChannelTreeByParentId(parentId, channelId int64) ([]*service_model.TreeNode, error) {
	out := make([]*service_model.TreeNode, 0)
	mdl, do := mapper.CmsSiteChannelDo()
