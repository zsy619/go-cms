...
func (svc *WeixinMpVerify) Get(accountId int64) ([]*domain.WeixinMpVerify, error) {
	mdl, do := mapper.WeixinMpVerifyDo()
 ...
 * @return {*}
 */
func (svc *WeixinMpVerify) GetCache() ([]*domain.WeixinMpVerify, error) {
	...
	if found, item := lib.WechatVerifyCache.Get(cacheKey); found {
		list := item.([]*domain.WeixinMpVerify)
...
func (svc *WeixinMpVerify) Find(verifyId int64) (*domain.WeixinMpVerify, error) {
	mdl, do := mapper.WeixinMpVerifyDo()
	return do.Where(mdl.VerifyID.Eq(verifyId)).First()
}

func (svc *WeixinMpVerify) Save(m *domain.WeixinMpVerify) error {
