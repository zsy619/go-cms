...
// ThemePaginate 分页查询
func (svc *CmsTheme) ThemePaginate(page, limit int, name, title string) ([]*domain.CmsTheme, int64, error) {
	mdl, do := mapper.CmsThemeDo()
	if name != "" {
...
// ThemeFind 获取
func (svc *CmsTheme) ThemeFind(themeId int64) (*domain.CmsTheme, error) {
	mdl, do := mapper.CmsThemeDo()
	return do.Where(mdl.ThemeID.Eq(themeId)).First()
}

// ThemeSave 保存或更新
func (svc *CmsTheme) ThemeSave(input *domain.CmsTheme) error {
	mdl, do := mapper.CmsThemeDo()
	if input.Name != "" {
