...
// Paginate 分页查询
func (svc *PlgOnlineRegister) Paginate(page, limit int, realName, special, degree, tags, remark string, isRead int32) ([]*domain.PlgOnlineRegister, int64, error) {
	mdl, do := mapper.PlgOnlineRegisterDo()
	if realName != "" {
...
// Find 获取
func (svc *PlgOnlineRegister) Find(registerId int64) (*domain.PlgOnlineRegister, error) {
	mdl, do := mapper.PlgOnlineRegisterDo()
...
// LinkSave 保存或更新
func (svc *PlgOnlineRegister) Save(input *domain.PlgOnlineRegister) error {
	mdl, do := mapper.PlgOnlineRegisterDo()
	if input.Mobile != "" {
